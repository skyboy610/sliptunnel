...
cleanup_existing() {
    ...
    # FORCE free port 53 — kill anything that's not systemd-resolved
    for port in 53 5353 8053; do
        ...
        pid_on_port=$(fuser "${port}/tcp" 2>/dev/null | tr -d '[:space:]')
        if [[ -n "$pid_on_port" ]]; then
            local pname
            pname=$(ps -p "$pid_on_port" -o comm= 2>/dev/null || echo "unknown")
            if [[ "$pname" != "systemd-resolve" && "$pname" != "systemd-r" ]]; then
...
print_banner() {
    local Y='\033[38;5;226m'  # yellow text
    local BY='\033[48;5;226m' # yellow bg
    local BB='\033[48;5;33m'  # blue bg
    local FB='\033[38;5;33m'  # blue text
    ...
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ███████║███████╗██║   ██║   ██║  ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ╚══════╝╚══════╝╚═╝   ╚═╝   ╚═╝  ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${W}   SLIM TUNNEL  Server  v4        ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}└──────────────────────────────────────┘${RST}"
    echo ""
}
...
create_server_service() {
    ...
    # ALWAYS pass all flags directly — do NOT rely on --help detection
    # If flag not supported, binary errors out (visible in logs) — better than silent drop
    ...
    # reset-seed: pass file path (slipstream-rust reads from file)
...
show_dns_records() {
    ...
    local BY='\033[48;5;226m' # yellow bg
    local FY='\033[38;5;226m' # yellow text
    local FW='\033[38;5;255m' # white text
    local FT='\033[38;5;30m'  # teal text
    ...
    echo -e "  ${BY}${FT}├────────┼───────────────────────┼───────────────────────┤${RST}"
    printf "  ${BY}${FT}│${BT}${FW}${BOLD} %-7s${RST}${BY}${FT}│${BT}${FW} %-22s${RST}${BY}${FT}│${BT}${FW} %-22s${RST}${BY}${FT}│${RST}\n" "A" "${NS_HOSTNAME}" "${SERVER_IP}"
    echo -e "  ${BY}${FT}├────────┼───────────────────────┼───────────────────────┤${RST}"
...
print_banner() {
    ...
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ███████║███████╗██║   ██║   ██║  ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ╚══════╝╚══════╝╚═╝   ╚═╝   ╚═╝  ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${W}   SLIM TUNNEL  Iran Client  v4   ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}└──────────────────────────────────────┘${RST}"
    echo ""
}
...
create_client_service() {
    ...
    # ALWAYS pass flags directly — never rely on --help detection
    local exec_cmd="${client_bin}"
...
uninstall_abroad() {
    ...
    if confirm "Remove SlipTunnel completely?"; then
        ...
        for port in 53 5353 8053 2053 8388 5201 5202; do
            local pids
            pids=$(fuser "${port}/udp" 2>/dev/null | tr -d '[:space:]')
