...
print_banner() {
    ...
    local BB='\033[48;5;33m'  # blue bg
    local FB='\033[38;5;33m'  # blue text (border)
    local FY='\033[38;5;226m' # yellow on blue
    local W='\033[38;5;255m'  # white
    clear
    echo ""
    echo -e "  ${BY}${FB}┌──────────────────────────────────────────────────────────────────────────┐${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ███████╗██╗     ██╗██████╗ ████████╗██╗   ██╗███╗  ██╗███╗  ██╗███████╗██╗  ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ██╔════╝██║     ██║██╔══██╗╚══██╔══╝██║   ██║████╗ ██║████╗ ██║██╔════╝██║  ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ███████╗██║     ██║██████╔╝   ██║   ██║   ██║██╔██╗██║██╔██╗██║█████╗  ██║  ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ╚════██║██║     ██║██╔═══╝    ██║   ██║   ██║██║╚████║██║╚████║██╔══╝  ██║  ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ███████║███████╗██║██║        ██║   ╚██████╔╝██║ ╚███║██║ ╚███║███████╗███████╗${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${FY}${BOLD}  ╚══════╝╚══════╝╚═╝╚═╝        ╚═╝    ╚═════╝ ╚═╝  ╚══╝╚═╝  ╚══╝╚══════╝╚══════╝${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}│${BB}${W}                        DNS Covert Channel Deployer  —  Server  v4.0                   ${RST}${BY}${FB}│${RST}"
    echo -e "  ${BY}${FB}└──────────────────────────────────────────────────────────────────────────┘${RST}"
    echo ""
}
...
# ═══ Fixed systemd service — ALWAYS pass flags directly ═══
create_server_service() {
    step "Creating systemd service..."

    local server_bin="$INSTALL_DIR/bin/slipstream-server"

    # ── Detect correct flag name for DNS listen port ──
    local dns_flag="--dns-listen-port"
    if "$server_bin" --help 2>&1 | grep -q -- "--listen"; then
        dns_flag="--listen"
    elif "$server_bin" --help 2>&1 | grep -q -- "--bind"; then
        dns_flag="--bind"
    fi

    local exec_cmd="${server_bin}"
    if [[ "$dns_flag" == "--listen" || "$dns_flag" == "--bind" ]]; then
        exec_cmd+=" ${dns_flag} 0.0.0.0:${DNS_PORT}"
    else
        exec_cmd+=" ${dns_flag} ${DNS_PORT}"
    fi
...
EOF
    ...
    if systemctl is-active --quiet slipstream-server; then
        ...
        journalctl -u slipstream-server --no-pager -n 20 2>/dev/null | while read -r line; do
            echo -e "    ${DIM}${line}${RST}"
        done
        error "Port 53 is required. Fix the issue above and re-run option 1."
        error "Debug command: journalctl -u slipstream-server -n 50 --no-pager"
    fi
}
...
print_banner() {
    local BY='\033[48;5;226m'
    local BT='\033[48;5;30m'
    local FT='\033[38;5;30m'
    local FY='\033[38;5;226m'
    local W='\033[38;5;255m'
    local FD='\033[38;5;232m'
    clear
    echo ""
    echo -e "  ${BY}${FT}┌──────────────────────────────────────────────────────────────────────────┐${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ███████╗██╗     ██╗██████╗ ████████╗██╗   ██╗███╗  ██╗███╗  ██╗███████╗██╗  ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ██╔════╝██║     ██║██╔══██╗╚══██╔══╝██║   ██║████╗ ██║████╗ ██║██╔════╝██║  ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ███████╗██║     ██║██████╔╝   ██║   ██║   ██║██╔██╗██║██╔██╗██║█████╗  ██║  ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ╚════██║██║     ██║██╔═══╝    ██║   ██║   ██║██║╚████║██║╚████║██╔══╝  ██║  ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ███████║███████╗██║██║        ██║   ╚██████╔╝██║ ╚███║██║ ╚███║███████╗███████╗${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${FY}${BOLD}  ╚══════╝╚══════╝╚═╝╚═╝        ╚═╝    ╚═════╝ ╚═╝  ╚══╝╚═╝  ╚══╝╚══════╝╚══════╝${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}│${BT}${W}                          DNS Covert Channel Deployer  —  Iran Client  v4.0                ${RST}${BY}${FT}│${RST}"
    echo -e "  ${BY}${FT}└──────────────────────────────────────────────────────────────────────────┘${RST}"
    echo ""
}
...
create_client_service() {
    ...
    # Detect correct flag for listen port
    local listen_flag="--tcp-listen-port"
    if "$client_bin" --help 2>&1 | grep -q -- "--listen"; then
        listen_flag="--listen"
    elif "$client_bin" --help 2>&1 | grep -q -- "--bind"; then
        listen_flag="--bind"
    fi

    local exec_cmd="${client_bin}"
    if [[ "$listen_flag" == "--listen" || "$listen_flag" == "--bind" ]]; then
        exec_cmd+=" ${listen_flag} 127.0.0.1:${LOCAL_LISTEN_PORT}"
    else
        exec_cmd+=" ${listen_flag} ${LOCAL_LISTEN_PORT}"
    fi
    exec_cmd+=" --domain ${TUNNEL_DOMAIN}"
    exec_cmd+=" --cert ${CONFIG_DIR}/cert.pem"
    exec_cmd+=" ${RESOLVER_FLAGS}"
...
KEOF
    ...
    if systemctl is-active --quiet slipstream-client; then
        ...
        journalctl -u slipstream-client --no-pager -n 8 2>/dev/null | while read -r line; do
            echo -e "    ${DIM}${line}${RST}"
        done
