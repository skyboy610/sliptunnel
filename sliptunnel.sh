...
cleanup_existing() {
    ...
    # FORCE free port 53 — kill anything holding it
    for proto in udp tcp; do
        local pids
        pids=$(ss -${proto:0:1}lnp 2>/dev/null | awk '/:53 /{print $NF}' | grep -oP 'pid=\K[0-9]+' | sort -u)
        for pid in $pids; do
            local pname
            pname=$(ps -p "$pid" -o comm= 2>/dev/null || echo "unknown")
            if [[ "$pname" != "systemd-resolve" && "$pname" != "systemd-r" ]]; then
                warn "Killing ${pname} (pid ${pid}) on port 53/${proto}"
...
print_banner() {
    local CR='\033[38;5;161m'   # crimson/maroon text
    local BG='\033[48;5;240m'   # gray background
    local FG='\033[38;5;240m'   # gray text (for border fg on crimson bg)
    local BCR='\033[48;5;161m'  # crimson background (border)
    local W='\033[38;5;255m'    # white
    local DK='\033[38;5;236m'   # dark gray text
    clear
    echo ""
    echo -e "  ${BCR}${FG}┌──────────────────────────────────────────────────────┐${RST}"
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}                                                      ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}   ███████╗██╗     ██╗██████╗     ████████╗██╗   ██╗  ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}   ██╔════╝██║     ██║██╔══██╗    ╚══██╔══╝██║   ██║  ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}   ███████╗██║     ██║██████╔╝       ██║   ██║   ██║  ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}   ╚════██║██║     ██║██╔═══╝        ██║   ██║   ██║  ${RST}${BCR}${FG}│${RST}"
    ...
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}   ╚══════╝╚══════╝╚═╝╚═╝            ╚═╝    ╚═════╝   ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${CR}${BOLD}                                                      ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${DK}        Slipstream Tunnel V4          By SkyBoy610   ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}└──────────────────────────────────────────────────────┘${RST}"
    echo ""
}
...
# ═══════════════════════════════════════════════════════════════
#  BUG FIX #2: Disable systemd-resolved if conflicting — FORCE port 53
# ═══════════════════════════════════════════════════════════════

fix_port53_conflict() {
    step "Freeing port 53 (mandatory)..."

    # ── Kill systemd-resolved stub listener ──
    if systemctl is-active --quiet systemd-resolved 2>/dev/null; then
        substep "Disabling systemd-resolved stub listener..."
...
RESOLVEDEOF
    ...
    # ── Kill anything else still on port 53 (UDP and TCP) ──
    for proto in udp tcp; do
        local pids
        pids=$(ss -${proto:0:1}lnp 2>/dev/null | awk '/:53 /{print $NF}' | grep -oP 'pid=\K[0-9]+' | sort -u)
        if [[ -n "$pids" ]]; then
            for pid in $pids; do
                local pname
                pname=$(ps -p "$pid" -o comm= 2>/dev/null || echo "unknown")
                warn "Killing '${pname}' (pid ${pid}) holding port 53/${proto}"
                kill -9 "$pid" 2>/dev/null || true
            done
            sleep 1
        fi
    ...
    # ── Also try fuser as backup method ──
    fuser -k 53/udp 2>/dev/null || true
    fuser -k 53/tcp 2>/dev/null || true
    sleep 1
    ...
    if ss -ulnp 2>/dev/null | grep -q ":53 "; then
        local still
        still=$(ss -ulnp 2>/dev/null | grep ":53 " | awk '{print $NF}' | head -1)
        error "Port 53/UDP still occupied by: ${still}"
        error "Cannot continue — port 53 is REQUIRED. Free it manually and re-run."
        exit 1
    fi

    success "Port 53 is free and open"
    return 0
}

auto_select_dns_port() {
    # DNS tunnel MUST run on port 53 — no fallback allowed
    DNS_PORT="53"
    fix_port53_conflict
    success "DNS Port: 53 (fixed)"
}
...
build_slipstream() {
    ...
    # ── Show actual flags for debug ──
    substep "Server binary flags:"
    "$INSTALL_DIR/bin/slipstream-server" --help 2>&1 | head -20 | while read -r line; do
        echo "    $line"
    done
...
# ═══ CRITICAL FIX: cert CN must match TUNNEL_DOMAIN exactly ═══
# slipstream-client verifies server cert hostname against TUNNEL_DOMAIN
# If CN != TUNNEL_DOMAIN → TLS error 0x433 (CERTIFICATE_VERIFICATION_FAILED)
generate_certs() {
    step "Generating TLS certificates..."
    mkdir -p "$CONFIG_DIR"

    # CN MUST be the tunnel domain — this is what the client verifies
    local cert_cn="${TUNNEL_DOMAIN}"
    if [[ -z "$cert_cn" ]]; then
        error "TUNNEL_DOMAIN not set — cannot generate cert!"
        exit 1
    fi

    substep "Generating cert with CN=${cert_cn}"

    # Try EC key first (smaller, faster), then RSA fallback
    if openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 -nodes \
        ...
        -days 3650 \
        -subj "/CN=${cert_cn}" \
        -addext "subjectAltName=DNS:${cert_cn},DNS:*.${cert_cn}" \
        2>/dev/null; then
        substep "EC P-256 cert generated"
    else
        # -addext not supported on old openssl — use config file
        substep "Trying RSA with config file..."
        local ssl_conf="/tmp/slipstream-ssl-$$.conf"
        cat > "$ssl_conf" <<SSLEOF
[req]
...
CN = ${cert_cn}
[v3_req]
subjectAltName = DNS:${cert_cn},DNS:*.${cert_cn}
basicConstraints = CA:TRUE
SSLEOF
        openssl req -x509 -newkey rsa:2048 -nodes \
            -keyout "$CONFIG_DIR/key.pem" -out "$CONFIG_DIR/cert.pem" \
            -days 3650 -config "$ssl_conf" 2>/dev/null || \
        openssl req -x509 -newkey rsa:2048 -nodes \
            -keyout "$CONFIG_DIR/key.pem" -out "$CONFIG_DIR/cert.pem" \
            -days 3650 -subj "/CN=${cert_cn}" 2>/dev/null
        rm -f "$ssl_conf"
        substep "RSA cert generated"
    fi

    # Verify cert was created and has correct CN
    local actual_cn
    actual_cn=$(openssl x509 -noout -subject -in "$CONFIG_DIR/cert.pem" 2>/dev/null | sed 's/.*CN\s*=\s*//' | xargs)
    if [[ "$actual_cn" != "$cert_cn" ]]; then
        warn "CN mismatch: got '${actual_cn}' expected '${cert_cn}' — trying simple method..."
        openssl req -x509 -newkey rsa:2048 -nodes \
            -keyout "$CONFIG_DIR/key.pem" -out "$CONFIG_DIR/cert.pem" \
            -days 3650 -subj "/CN=${cert_cn}" 2>/dev/null
    fi

    # Binary expects exactly 32 hex chars = 16 bytes
    openssl rand -hex 16 > "$CONFIG_DIR/reset-seed"
    chmod 600 "$CONFIG_DIR/key.pem" "$CONFIG_DIR/reset-seed"

    # Show fingerprint for verification
    local fp
    fp=$(openssl x509 -noout -fingerprint -sha256 -in "$CONFIG_DIR/cert.pem" 2>/dev/null | cut -d= -f2)
    success "Certificate generated: CN=${cert_cn}"
    substep "SHA256: ${fp}"
    substep "Cert will be bundled into Iran package for mutual verification"
}
...
create_server_service() {
    ...
    # ── Read reset-seed VALUE (exactly 32 hex chars) ──
    local reset_seed_val
    reset_seed_val=$(cat "$CONFIG_DIR/reset-seed" 2>/dev/null | tr -d '[:space:]')
    if [[ -z "$reset_seed_val" ]] || [[ ${#reset_seed_val} -ne 32 ]]; then
        reset_seed_val=$(openssl rand -hex 16)
        printf '%s' "$reset_seed_val" > "$CONFIG_DIR/reset-seed"
    fi
    substep "Reset seed: ${#reset_seed_val} hex chars ✓"

    # ── Probe binary --help (captures both stdout+stderr) ──
    local help_out
    help_out=$("$server_bin" --help 2>&1 || "$server_bin" -h 2>&1 || true)
    substep "Binary help (first 20 lines):"
    echo "$help_out" | head -20 | while read -r line; do echo "      $line"; done
    echo ""

    # ── Build ExecStart ──
    local exec_cmd="${server_bin}"
    local used_flags=false

    # -- Listen address --
    if   echo "$help_out" | grep -qE -- "--listen[^-]";       then exec_cmd+=" --listen 0.0.0.0:${DNS_PORT}"; used_flags=true
    elif echo "$help_out" | grep -qE -- "--bind[^-]";         then exec_cmd+=" --bind 0.0.0.0:${DNS_PORT}"; used_flags=true
    elif echo "$help_out" | grep -q  -- "--dns-listen-port";  then exec_cmd+=" --dns-listen-port ${DNS_PORT}"; used_flags=true
    elif echo "$help_out" | grep -qE -- "--port[^-]";         then exec_cmd+=" --port ${DNS_PORT}"; used_flags=true
    else exec_cmd+=" --listen 0.0.0.0:${DNS_PORT}"; fi  # default

    # -- Domain (flag OR positional) --
    if   echo "$help_out" | grep -q -- "--domain";  then exec_cmd+=" --domain ${TUNNEL_DOMAIN}"
    else exec_cmd+=" ${TUNNEL_DOMAIN}"; fi  # positional fallback

    # -- Target (flag OR positional) --
    if   echo "$help_out" | grep -q -- "--target-address"; then exec_cmd+=" --target-address ${TARGET_ADDR}"
    elif echo "$help_out" | grep -q -- "--target";         then exec_cmd+=" --target ${TARGET_ADDR}"
    elif echo "$help_out" | grep -q -- "--upstream";       then exec_cmd+=" --upstream ${TARGET_ADDR}"
    else exec_cmd+=" ${TARGET_ADDR}"; fi  # positional fallback

    # -- Cert & Key --
    if echo "$help_out" | grep -q -- "--cert";       then exec_cmd+=" --cert ${CONFIG_DIR}/cert.pem"; fi
    if echo "$help_out" | grep -q -- "--key";        then exec_cmd+=" --key ${CONFIG_DIR}/key.pem"; fi

    # -- Reset seed (inline value, NOT file path) --
    if echo "$help_out" | grep -q -- "--reset-seed"; then exec_cmd+=" --reset-seed ${reset_seed_val}"; fi
...
EOF
    ...
    if systemctl is-active --quiet slipstream-server; then
        success "slipstream-server RUNNING on port ${DNS_PORT}"
    else
        error "Service failed to start!"
        echo ""
        warn "Last 25 log lines:"
        journalctl -u slipstream-server --no-pager -n 25 2>/dev/null | while read -r line; do
            echo -e "    ${DIM}${line}${RST}"
        done
        echo ""
        error "Installation aborted. Fix the error above and re-run option 1."
        error "Debug: journalctl -u slipstream-server -n 50 --no-pager"
        exit 1
    fi
}
...
show_dns_records() {
    local BT='\033[48;5;30m'   # teal bg
    local BY='\033[48;5;226m'  # yellow bg
    local FT='\033[38;5;30m'   # teal text (border)
    local FY='\033[38;5;226m'  # yellow text (header)
    local FW='\033[38;5;255m'  # white text (labels)
    local FD='\033[38;5;232m'  # near-black text (IP values — dark on teal)

    echo ""
    echo -e "  ${BOLD}${YELLOW}⚠ REQUIRED DNS Records${RST}"
    echo ""
    echo -e "  ${BY}${FT}┌────────┬───────────────────────┬───────────────────────┐${RST}"
    printf "  ${BY}${FT}│${BT}${FY}${BOLD} %-7s${RST}${BY}${FT}│${BT}${FY}${BOLD} %-22s${RST}${BY}${FT}│${BT}${FY}${BOLD} %-22s${RST}${BY}${FT}│${RST}\n" "Type" "Name" "Value"
    echo -e "  ${BY}${FT}├────────┼───────────────────────┼───────────────────────┤${RST}"
    printf "  ${BY}${FT}│${BT}${FW}${BOLD} %-7s${RST}${BY}${FT}│${BT}${FW} %-22s${RST}${BY}${FT}│${BT}${BOLD}${FD} %-22s${RST}${BY}${FT}│${RST}\n" "A" "${NS_HOSTNAME}" "${SERVER_IP}"
    echo -e "  ${BY}${FT}├────────┼───────────────────────┼───────────────────────┤${RST}"
    ...
    if [[ "$DNS_PORT" != "53" ]]; then
        warn "DNS port: ${DNS_PORT} (non-standard)"
        error "Recursive resolvers use port 53 only!"
        echo -e "  ${WHITE}  Iran MUST use: ${CYAN}${SERVER_IP}:${DNS_PORT}${RST}"
        echo ""
    fi
...
verify_server() {
    ...
    # Wait up to 15s for service to fully start
    local attempt=0
    ...
    if systemctl is-active --quiet slipstream-server; then
        ...
        journalctl -u slipstream-server --no-pager -n 10 2>/dev/null | while read -r line; do
            echo -e "    ${DIM}${line}${RST}"
        done
        echo ""
        ok=false
    ...
    if ss -ulnp 2>/dev/null | grep -q ":${DNS_PORT} "; then
        ...
        warn "Port ${DNS_PORT}/UDP: not yet listening (service may need a moment)"
    fi
...
create_iran_package() {
    ...
    # Save cert fingerprint so iran client can verify cert integrity
    openssl x509 -noout -fingerprint -sha256 -in "$CONFIG_DIR/cert.pem" 2>/dev/null \
        | cut -d= -f2 > "$PACKAGE_DIR/config/cert.fingerprint" || true
...
EOF
    if [[ "$TUNNEL_MODE" == "shadowsocks" ]]; then
        {
            echo "SS_PASSWORD=$(printf '%q' "${SS_PASSWORD}")"
            echo "SS_METHOD=${SS_METHOD}"
            echo "SS_PORT=${SS_PORT}"
        } >> "$PACKAGE_DIR/config/connection.conf"
    fi
...
# ═══ Real Tunnel Test — test resolver with actual slipstream-client ═══
test_tunnel_real() {
    local resolver_ip="$1" resolver_port="${2:-53}" test_timeout="${3:-8}"
    local client_bin="$INSTALL_DIR/bin/slipstream-client"
    local test_port=19999
    local test_cert="${CONFIG_DIR}/cert.pem"
    local test_domain="${TUNNEL_DOMAIN:-}"

    [[ ! -x "$client_bin" ]] && return 1
    [[ -z "$test_domain" ]] && return 1
    [[ ! -f "$test_cert" ]] && return 1

    # Find free test port
    while ss -tlnp 2>/dev/null | grep -q ":${test_port} "; do
        test_port=$((test_port + 1))
        [[ $test_port -gt 20100 ]] && return 1
    done

    # Run client temporarily in background
    timeout "$test_timeout" "$client_bin" \
        --tcp-listen-port "$test_port" \
        --domain "$test_domain" \
        --cert "$test_cert" \
    ...
    # Wait for port to appear (means connection worked)
    local waited=0
    while [[ $waited -lt $test_timeout ]]; do
        sleep 1
        waited=$((waited + 1))
        if ss -tlnp 2>/dev/null | grep -q ":${test_port} "; then
            # Port is listening = tunnel connected!
...
# ═══ Benchmark with real tunnel test ═══
benchmark_with_tunnel_test() {
    [[ -f "$CONFIG_DIR/client.conf" ]] && source "$CONFIG_DIR/client.conf"

    if [[ -z "${TUNNEL_DOMAIN:-}" ]]; then
        warn "No tunnel domain configured — can't test tunnel"
        return 1
    fi

    # First run standard benchmark
    if ! benchmark_resolvers; then
        return 1
    fi

    step "Phase 2: Real tunnel test on top resolvers..."
    echo ""

    local tunnel_ok=() tunnel_fail=0 test_count=0
    local max_test=10  # Test top 10 from benchmark

    for entry in "${BENCHMARK_RESULTS[@]}"; do
        [[ $test_count -ge $max_test ]] && break
        ...
        printf "    ${BLUE}→${RST} ${DIM}Testing tunnel: ${r_ip}:${r_port} [${test_count}/${max_test}]...${RST}"

        if test_tunnel_real "$r_ip" "$r_port" 10; then
            echo -e "\r    ${GREEN}✔${RST} ${r_ip}:${r_port} — ${BOLD}${GREEN}TUNNEL WORKS${RST}           "
            tunnel_ok+=("${entry}")
        else
            echo -e "\r    ${RED}✘${RST} ${r_ip}:${r_port} — ${RED}tunnel failed${RST}              "
    ...
    if [[ ${#tunnel_ok[@]} -eq 0 ]]; then
        error "No resolvers passed tunnel test!"
        echo -e "  ${YELLOW}  Check: DNS records, certificate match, server status${RST}"
        return 1
    fi

    success "${#tunnel_ok[@]} resolvers passed REAL tunnel test"
    [[ $tunnel_fail -gt 0 ]] && warn "${tunnel_fail} failed tunnel test"

    BENCHMARK_RESULTS=("${tunnel_ok[@]}")
    return 0
}

# ═══ Import custom IP list for testing ═══
import_custom_resolvers() {
    step "Import Custom Resolver List"
    echo ""
    echo -e "  ${MC1}${BOLD}1)${RST} ${MC1}Paste IPs (comma/space/newline)${RST}"
    echo -e "  ${MC2}${BOLD}2)${RST} ${MC2}Load from file${RST}"
    echo ""
    local import_choice
    ask_input "Select" "1" import_choice

    local custom_ips=""

    if [[ "$import_choice" == "2" ]]; then
        local filepath
        ask_input "File path" "/root/dns-list.txt" filepath
        if [[ -f "$filepath" ]]; then
            custom_ips=$(grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' "$filepath" | sort -u | tr '\n' ',')
            local count
            count=$(echo "$custom_ips" | tr ',' '\n' | grep -c '.')
            success "Loaded ${count} IPs from ${filepath}"
        else
            error "File not found: ${filepath}"
            return 1
        fi
    else
        echo -e "  ${CYAN}  Paste IPs, press Enter on empty line to finish:${RST}"
        local line_in=""
        while true; do
            echo -ne "  ${ORANGE}>${RST} "
            read -r line_in
            [[ -z "$line_in" ]] && break
            [[ -n "$custom_ips" ]] && custom_ips+=","
            custom_ips+="$(echo "$line_in" | tr ' \t' ',')"
        done
    fi

    if [[ -z "$custom_ips" ]]; then
        warn "No IPs provided"
        return 1
    fi

    # Build temporary resolver array
    local old_resolvers=("${IRAN_RESOLVERS[@]}")
    IRAN_RESOLVERS=()

    IFS=',' read -ra ip_arr <<< "$custom_ips"
    for ip in "${ip_arr[@]}"; do
        ip=$(echo "$ip" | xargs 2>/dev/null || echo "$ip")
        [[ -z "$ip" ]] && continue
        local ip_clean="${ip%%:*}"
    ...
    local total=${#IRAN_RESOLVERS[@]}
    success "Testing ${total} custom resolvers..."
    echo ""
    ...
    IRAN_RESOLVERS=("${old_resolvers[@]}")
}

# ═══ Auto-replenish resolvers (called by watchdog) ═══
auto_replenish_resolvers() {
    # Check how many current resolvers are alive
    [[ -f "$CONFIG_DIR/client.conf" ]] && source "$CONFIG_DIR/client.conf"
    [[ -z "${RESOLVERS:-}" ]] && return 0

    local alive=0 total=0
    IFS=',' read -ra cur_resolvers <<< "$RESOLVERS"
    total=${#cur_resolvers[@]}

    for rv in "${cur_resolvers[@]}"; do
        local rip="${rv%%:*}" rport="${rv##*:}"
        if timeout 2 bash -c "echo >/dev/tcp/${rip}/${rport}" 2>/dev/null; then
            alive=$((alive + 1))
        fi
    done

    # If 1 or fewer alive, replenish
    if [[ $alive -le 1 ]]; then
        echo "[$(date -Iseconds)] REPLENISH: only ${alive}/${total} resolvers alive" >> /var/log/slipstream/watchdog.log

        # Stop tunnel safely
        systemctl stop slipstream-client 2>/dev/null || true
        sleep 2

        # Quick benchmark (reduced timeout)
        if benchmark_resolvers 2; then
            select_best_resolvers 5
        ...
        # Restart tunnel
        systemctl start slipstream-client 2>/dev/null
        sleep 3

        if systemctl is-active --quiet slipstream-client; then
            echo "[$(date -Iseconds)] REPLENISH: tunnel restarted with new resolvers: ${RESOLVERS}" >> /var/log/slipstream/watchdog.log
        else
            echo "[$(date -Iseconds)] REPLENISH: tunnel restart FAILED" >> /var/log/slipstream/watchdog.log
        fi
    fi
}

# ═══ Anti-DPI periodic restart ═══
setup_anti_dpi_restart() {
    print_banner
    step "Anti-DPI Periodic Restart"
    echo ""

    if systemctl is-active --quiet slipstream-restart.timer 2>/dev/null; then
        echo -e "  ${GREEN}●${RST} Anti-DPI restart: ${GREEN}${BOLD}ACTIVE${RST}"
        local current_interval
        current_interval=$(systemctl show slipstream-restart.timer -p TimersCalendar --value 2>/dev/null || echo "unknown")
        echo -e "  ${WHITE}  Current: ${LIME}${current_interval}${RST}"
        echo ""
        if confirm "Disable?"; then
            systemctl stop slipstream-restart.timer 2>/dev/null
            systemctl disable slipstream-restart.timer 2>/dev/null
            rm -f /etc/systemd/system/slipstream-restart.*
            systemctl daemon-reload
            success "Disabled"
        fi
    else
        echo -e "  ${RED}●${RST} Anti-DPI restart: ${RED}${BOLD}INACTIVE${RST}"
        echo ""
        echo -e "  ${WHITE}  Restart tunnel periodically to avoid DPI detection${RST}"
        echo ""
        echo -e "  ${MC1}${BOLD}1)${RST} ${MC1}Every 30 minutes${RST}"
        echo -e "  ${MC2}${BOLD}2)${RST} ${MC2}Every 1 hour${RST}"
        echo -e "  ${MC3}${BOLD}3)${RST} ${MC3}Every 2 hours${RST}"
        echo -e "  ${MC4}${BOLD}4)${RST} ${MC4}Custom (minutes)${RST}"
        echo ""
        local dpi_choice
        ask_input "Select" "2" dpi_choice
        ...
        case "$dpi_choice" in
            ...
            2) minutes=60 ;;
            3) minutes=120 ;;
            4) ask_input "Minutes" "60" minutes ;;
        esac

        cat > /etc/systemd/system/slipstream-restart.service <<DPISVC
[Unit]
Description=SlipTunnel Anti-DPI Restart
[Service]
Type=oneshot
ExecStart=/bin/systemctl restart slipstream-client
DPISVC

        cat > /etc/systemd/system/slipstream-restart.timer <<DPITMR
[Unit]
Description=SlipTunnel Anti-DPI Timer
[Timer]
...
DPITMR
        ...
        systemctl daemon-reload
        systemctl enable --now slipstream-restart.timer >/dev/null 2>&1
        success "Enabled: restart every ${minutes} minutes"
    fi
    echo ""
}
...
print_banner() {
    local CR='\033[38;5;161m'   # crimson text
    local BG='\033[48;5;240m'   # gray background
    local FG='\033[38;5;240m'   # gray (for border fg on crimson bg)
    local BCR='\033[48;5;161m'  # crimson background (border)
    local W='\033[38;5;255m'    # white
    local DK='\033[38;5;236m'   # dark gray
    local GR='\033[38;5;245m'   # gray text (inverted: gray text on gray bg)
    clear
    echo ""
    echo -e "  ${BCR}${FG}┌──────────────────────────────────────────────────────┐${RST}"
    echo -e "  ${BCR}${FG}│${BG}${GR}${BOLD}                                                      ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${GR}${BOLD}   ███████╗██╗     ██╗██████╗     ████████╗██╗   ██╗  ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${GR}${BOLD}   ██╔════╝██║     ██║██╔══██╗    ╚══██╔══╝██║   ██║  ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${GR}${BOLD}   ███████╗██║     ██║██████╔╝       ██║   ██║   ██║  ${RST}${BCR}${FG}│${RST}"
    echo -e "  ${BCR}${FG}│${BG}${GR}${BOLD}   ╚════██║██║     ██║██╔═══╝        ██║   ██║   ██║  ${RST}${BCR}${FG}│${RST}"
...
install_client() {
    ...
    # ── Verify cert integrity if fingerprint available ──
    if [[ -f "$SCRIPT_DIR/config/cert.fingerprint" && -f "$CONFIG_DIR/cert.pem" ]]; then
        local expected_fp actual_fp
        expected_fp=$(cat "$SCRIPT_DIR/config/cert.fingerprint" | tr -d '[:space:]')
        actual_fp=$(openssl x509 -noout -fingerprint -sha256 -in "$CONFIG_DIR/cert.pem" 2>/dev/null | cut -d= -f2 | tr -d '[:space:]')
        if [[ "$expected_fp" == "$actual_fp" ]]; then
            success "cert.pem verified ✓ (SHA256 matches)"
        else
            error "cert.pem MISMATCH!"
            error "Expected: ${expected_fp}"
            error "Got:      ${actual_fp}"
            error "The cert in this package does not match the server cert!"
            error "Re-generate the Iran package from the foreign server (option 2)"
            exit 1
        fi
    fi

    # Save full resolver pool for watchdog auto-rotation
...
configure_client() {
    ...
    # Also strip IPv4-mapped IPv6 prefix [::ffff:] if present
    local fixed=""
    local clean_input
    ...
    while IFS= read -r r; do
        ...
        r="${r%\]}"
        r="${r#::ffff:}"
        # Validate it looks like an IP:port or IP
        local ip_part="${r%%:*}"
    ...
    # Save config — quote SS_PASSWORD to handle special chars (/, $, !, etc)
...
CEOF
    if [[ "$TUNNEL_MODE" == "shadowsocks" ]]; then
        # Write SS_PASSWORD safely — avoid heredoc expansion of special chars
        {
            echo "SS_PASSWORD=$(printf '%q' "${SS_PASSWORD:-}")"
            echo "SS_METHOD=${SS_METHOD:-chacha20-ietf-poly1305}"
            echo "SS_PORT=${SS_PORT:-8388}"
        } >> "$CONFIG_DIR/client.conf"
    fi
    chmod 600 "$CONFIG_DIR/client.conf"
    # Ensure resolver pool exists for watchdog
    if [[ ${#IRAN_RESOLVERS[@]} -gt 0 && ! -f "$CONFIG_DIR/resolver-pool.txt" ]]; then
        printf '%s\n' "${IRAN_RESOLVERS[@]}" > "$CONFIG_DIR/resolver-pool.txt"
        chmod 600 "$CONFIG_DIR/resolver-pool.txt"
    fi
...
create_client_service() {
    step "Creating client service..."

    local client_bin="$INSTALL_DIR/bin/slipstream-client"

    # ── Rebuild RESOLVER_FLAGS from RESOLVERS (in case loaded from config) ──
    if [[ -z "$RESOLVER_FLAGS" && -n "$RESOLVERS" ]]; then
        local rf=""
        IFS=',' read -ra _rarr <<< "$RESOLVERS"
        for _r in "${_rarr[@]}"; do
            _r=$(echo "$_r" | xargs)
            [[ -n "$_r" ]] && rf+="--resolver ${_r} "
        done
        RESOLVER_FLAGS="${rf% }"  # trim trailing space
    fi

    # ── Probe binary --help ──
    local help_out
    help_out=$("$client_bin" --help 2>&1 || true)

    local exec_cmd="${client_bin}"

    # -- Listen port --
    if   echo "$help_out" | grep -q -- "--tcp-listen-port"; then exec_cmd+=" --tcp-listen-port ${LOCAL_LISTEN_PORT}"
    elif echo "$help_out" | grep -q -- "--listen";          then exec_cmd+=" --listen 127.0.0.1:${LOCAL_LISTEN_PORT}"
    elif echo "$help_out" | grep -q -- "--bind";            then exec_cmd+=" --bind 127.0.0.1:${LOCAL_LISTEN_PORT}"
    elif echo "$help_out" | grep -q -- "--port";            then exec_cmd+=" --port ${LOCAL_LISTEN_PORT}"
    else exec_cmd+=" --tcp-listen-port ${LOCAL_LISTEN_PORT}"; fi
    ...
    # -- Resolvers (always append) --
    exec_cmd+=" ${RESOLVER_FLAGS% }"  # trim trailing space
...
KEOF
    ...
    sleep 5
    if systemctl is-active --quiet slipstream-client; then
        success "slipstream-client RUNNING on port ${LOCAL_LISTEN_PORT}"
    else
        warn "Service may still be starting up..."
        journalctl -u slipstream-client --no-pager -n 15 2>/dev/null | while read -r line; do
            echo -e "    ${DIM}${line}${RST}"
        done
        error "Debug: journalctl -u slipstream-client -n 50 --no-pager"
    fi
}
...
#!/usr/bin/env bash
# ═══ SlipTunnel Smart Watchdog v4 ═══
# - Auto-restart service if down
# - Count alive resolvers
# - Auto-rotate from pool when alive < MIN_ALIVE
...
MIN_ALIVE=3          # Rotate when fewer than this are alive
PICK_COUNT=10        # How many new resolvers to pick
TEST_BATCH=40        # Test this many from pool at once
MAX_LOG=1048576      # 1MB log limit
...
if ! systemctl is-active --quiet slipstream-client; then
    ...
    if systemctl is-active --quiet slipstream-client; then
        ...
        if ss -tlnp 2>/dev/null | grep -q ":${wd_port} "; then
            ts "ERROR: port ${wd_port} occupied, killing..."
            fuser -k "${wd_port}/tcp" 2>/dev/null
            sleep 2
            systemctl restart slipstream-client
            sleep 3
            systemctl is-active --quiet slipstream-client && ts "OK: recovered" || ts "ERROR: still failed"
        fi
    fi
fi

# ─── 2. Resolver health check ───
[[ ! -f "$CONF" ]] && exit 0
source "$CONF"
[[ -z "${RESOLVERS:-}" ]] && exit 0
...
for wd_r in "${wd_current[@]}"; do
    ...
    if timeout 3 bash -c "echo >/dev/tcp/${wd_ip}/${wd_rp}" 2>/dev/null; then
        # Quick recursive test if dig available
        if command -v dig &>/dev/null; then
            wd_rand="wdtest${RANDOM}${RANDOM}"
            wd_dig=$(dig "@${wd_ip}" -p "${wd_rp}" "${wd_rand}.google.com" A +time=2 +tries=1 +noall +comments 2>/dev/null)
            if [[ -n "$wd_dig" ]]; then
                wd_alive=$((wd_alive + 1))
                [[ -n "$wd_alive_list" ]] && wd_alive_list+=","
                wd_alive_list+="${wd_r}"
            else
                wd_dead=$((wd_dead + 1))
                ts "DEAD(recursive): ${wd_r}"
            fi
        else
            # No dig — TCP is enough
            wd_alive=$((wd_alive + 1))
            [[ -n "$wd_alive_list" ]] && wd_alive_list+=","
            wd_alive_list+="${wd_r}"
        fi
...
ts "STATUS: ${wd_alive} alive, ${wd_dead} dead out of ${#wd_current[@]}"

# ─── 3. Auto-rotate if too few alive ───
if [[ $wd_alive -lt $MIN_ALIVE && -f "$POOL" ]]; then
    ts "ROTATE: alive(${wd_alive}) < min(${MIN_ALIVE}), scanning pool..."

    # Read pool and shuffle
    mapfile -t wd_pool < "$POOL"
    wd_pool_size=${#wd_pool[@]}

    if [[ $wd_pool_size -eq 0 ]]; then
        ts "ERROR: resolver pool is empty"
        exit 1
    fi

    # Shuffle array (Fisher-Yates)
    for ((i=wd_pool_size-1; i>0; i--)); do
        j=$((RANDOM % (i+1)))
        wd_tmp="${wd_pool[$i]}"
        wd_pool[$i]="${wd_pool[$j]}"
        wd_pool[$j]="$wd_tmp"
    ...
    # Test a batch from shuffled pool
    wd_new_resolvers=""
    wd_found=0
    wd_tested=0

    for wd_entry in "${wd_pool[@]}"; do
        [[ $wd_found -ge $PICK_COUNT ]] && break
        [[ $wd_tested -ge $TEST_BATCH ]] && break
        wd_tested=$((wd_tested + 1))

        wd_eip="${wd_entry%%:*}"; wd_eport="${wd_entry##*:}"

        # Skip if already in alive list
        echo "$wd_alive_list" | grep -q "$wd_eip" && continue

        # TCP test
        if ! timeout 2 bash -c "echo >/dev/tcp/${wd_eip}/${wd_eport}" 2>/dev/null; then
            continue
        fi

        # Recursive test
        if command -v dig &>/dev/null; then
            wd_trand="rot${RANDOM}${RANDOM}"
            wd_tdig=$(dig "@${wd_eip}" -p "${wd_eport}" "${wd_trand}.google.com" A +time=2 +tries=1 +noall +comments 2>/dev/null)
            [[ -z "$wd_tdig" ]] && continue
        fi

        # Passed!
        [[ -n "$wd_new_resolvers" ]] && wd_new_resolvers+=","
        wd_new_resolvers+="${wd_eip}:${wd_eport}"
        wd_found=$((wd_found + 1))
        ts "FOUND: ${wd_eip}:${wd_eport}"
    done

    if [[ $wd_found -eq 0 ]]; then
        ts "ERROR: no working resolvers found in pool"
        exit 1
    fi

    # Merge: keep alive ones + add new ones
    wd_final=""
    [[ -n "$wd_alive_list" ]] && wd_final="${wd_alive_list},"
    wd_final+="${wd_new_resolvers}"
    # Remove trailing comma
    wd_final="${wd_final%,}"

    ts "ROTATE: new set = ${wd_final}"

    # Update config file
    sed -i "s|^RESOLVERS=.*|RESOLVERS=${wd_final}|" "$CONF"

    # Rebuild service ExecStart using --help detection
    wd_client="/opt/slipstream-tunnel/bin/slipstream-client"
    wd_help_out=$("$wd_client" --help 2>&1 || true)
    wd_exec="$wd_client"

    # Detect listen flag
    if echo "$wd_help_out" | grep -q -- "--tcp-listen-port"; then
        wd_exec+=" --tcp-listen-port ${LOCAL_LISTEN_PORT:-5201}"
    elif echo "$wd_help_out" | grep -q -- "--listen"; then
        wd_exec+=" --listen 127.0.0.1:${LOCAL_LISTEN_PORT:-5201}"
    elif echo "$wd_help_out" | grep -q -- "--bind"; then
        wd_exec+=" --bind 127.0.0.1:${LOCAL_LISTEN_PORT:-5201}"
    else
        wd_exec+=" --tcp-listen-port ${LOCAL_LISTEN_PORT:-5201}"
    fi

    # -- Detect domain flag (or positional) --
    if echo "$wd_help_out" | grep -q -- "--domain"; then
        wd_exec+=" --domain ${TUNNEL_DOMAIN}"
    else
        wd_exec+=" ${TUNNEL_DOMAIN}"  # positional fallback
    fi

    # Detect cert flag
    if echo "$wd_help_out" | grep -q -- "--cert"; then
        wd_exec+=" --cert /etc/slipstream/cert.pem"
    fi

    # Add resolver flags
    IFS=',' read -ra wd_new_arr <<< "$wd_final"
    for wd_nr in "${wd_new_arr[@]}"; do
        wd_exec+=" --resolver ${wd_nr}"
    done
    ...
    if systemctl is-active --quiet slipstream-client; then
        ts "ROTATE: SUCCESS — tunnel restarted with ${wd_found} new resolvers"
    else
        ts "ROTATE: FAILED — tunnel did not start"
    fi
fi
WD
...
show_menu() {
    ...
    echo -e "  ${MC3}${BOLD}13)${RST}  ${MC3}Real Tunnel Test${RST}"
    echo -e "  ${MC3}${BOLD}14)${RST}  ${MC3}Import Custom IP List${RST}"
    echo -e "  ${MC3}${BOLD}15)${RST}  ${MC3}Anti-DPI Periodic Restart${RST}"
    echo -e "  ${MC3}${BOLD}16)${RST}  ${MC3}Generate x-ui Config${RST}"
    echo -e "  ${MC4}${BOLD}17)${RST}  ${MC4}Uninstall${RST}"
            ...
            12) print_banner
                if benchmark_resolvers; then
                    ...
                    if confirm "Apply best resolvers and restart tunnel?"; then
                        ...
                        # Restart service with create_client_service (uses --help detection)
                        create_client_service
                ...
                echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            13) print_banner
                if benchmark_with_tunnel_test; then
                    echo ""
                    if confirm "Apply tunnel-tested resolvers?"; then
                        select_best_resolvers 5
                        RESOLVERS="$SELECTED_RESOLVERS"
                        RESOLVER_FLAGS="$SELECTED_RESOLVER_FLAGS"
                        [[ -f "$CONFIG_DIR/client.conf" ]] && sed -i "s|^RESOLVERS=.*|RESOLVERS=${RESOLVERS}|" "$CONFIG_DIR/client.conf"
                        create_client_service
                        systemctl is-active --quiet slipstream-client && success "Tunnel restarted" || error "Failed"
                    fi
                fi
                echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            14) import_custom_resolvers; echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            15) setup_anti_dpi_restart; echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            16) generate_xui_config; echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            17) uninstall_client; echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            0|q|Q) echo -e "\n  ${CYAN}Goodbye!${RST}\n"; exit 0 ;;
...
abroad_health() {
    ...
    if [[ -f "$CONFIG_DIR/cert.pem" ]]; then
        ...
        cert_cn=$(openssl x509 -in "$CONFIG_DIR/cert.pem" -noout -subject 2>/dev/null | sed 's/.*CN\s*=\s*//' | xargs)
        echo -e "  ${GREEN}✔${RST} Certificate valid until: ${exp}"; pass=$((pass+1))
        # CRITICAL: Check CN matches tunnel domain
        local conf_domain="${TUNNEL_DOMAIN:-}"
        if [[ -n "$conf_domain" && "$cert_cn" != "$conf_domain" ]]; then
            echo -e "  ${RED}✘${RST} ${BOLD}CERT CN MISMATCH!${RST}"
            echo -e "    ${RED}CN in cert:    ${cert_cn}${RST}"
            echo -e "    ${RED}Tunnel domain: ${conf_domain}${RST}"
            echo -e "    ${YELLOW}→ Client will get error 0x433! Run option 2 to regenerate cert.${RST}"
        else
            echo -e "  ${GREEN}✔${RST} Cert CN: ${cert_cn} ✓"
        fi
...
uninstall_abroad() {
    ...
    if confirm "Remove SlipTunnel completely?"; then
        ...
        for svc in slipstream-server slipstream-client shadowsocks-libev ss-backend danted microsocks slipstream-server-watchdog.timer slipstream-watchdog.timer; do
            systemctl stop "$svc" 2>/dev/null
            systemctl disable "$svc" 2>/dev/null
            rm -f "/etc/systemd/system/${svc}.service" "/etc/systemd/system/${svc}.timer" "/etc/systemd/system/${svc}"
        done
        systemctl daemon-reload 2>/dev/null

        # KILL anything on port 53 that's ours
        fuser -k 53/udp 2>/dev/null || true
        fuser -k 53/tcp 2>/dev/null || true
        for port in 8388 5201 5202; do
            local pids
            pids=$(fuser "${port}/tcp" 2>/dev/null | tr -d '[:space:]')
            [[ -n "$pids" ]] && kill -9 $pids 2>/dev/null || true
        done

        # Kill slipstream processes directly
        pkill -9 -f slipstream-server 2>/dev/null || true
        pkill -9 -f slipstream-client 2>/dev/null || true
        ...
        rm -f /usr/local/bin/st /usr/local/bin/slt /usr/local/bin/sliptunnel.sh
        rm -f /root/xui-outbound.json
        rm -f /root/slipstream-iran-*.tar.gz

        if confirm "Remove Rust toolchain?"; then
            rustup self uninstall -y 2>/dev/null || true
        fi
        echo ""; success "SlipTunnel completely removed (all ports freed)"
    fi; echo ""
}
...
show_main_menu() {
    ...
    echo -e "  ${MC1}${BOLD} 2)${RST}  ${MC1}Reconfigure + Regenerate Cert${RST}"
    ...
    while true; do
        ...
        case "$c" in
            1)  print_banner
                ...
                build_slipstream
                collect_config    # MUST be before generate_certs (needs TUNNEL_DOMAIN for CN)
                generate_certs    # CN = TUNNEL_DOMAIN
                setup_backend
                ...
                echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            2)  print_banner; collect_config
                generate_certs    # Regenerate cert with correct TUNNEL_DOMAIN CN
                setup_backend
                open_firewall_ports; create_server_service; save_config; show_dns_records
                warn "Iran package must be REBUILT to get new cert! Run option 3."
                echo -e "  ${DIM}Press Enter...${RST}"; read -r ;;
            3)  print_banner
