...
# ═══ Built-in Iran DNS Resolvers (50 tested, sorted by latency, diverse ISPs) ═══
...
IRAN_RESOLVERS=(
    ...
    "185.137.25.215:53:Faranet2"
    "178.22.126.2:53:AsiatechDC3"
    "185.137.27.40:53:Faranet3"
    "194.225.152.10:53:FundSci"
    "185.8.175.187:53:ParsParva2"
    "188.0.240.6:53:AsiatechDC4"
    "194.59.215.37:53:TadServer2"
    "185.112.150.24:53:Sefroyek3"
    "185.8.174.140:53:ParsParva3"
    "188.0.240.4:53:AsiatechDC5"
    "109.230.204.66:53:ParsNet2"
    "185.126.1.166:53:Pishgaman2"
    "185.147.161.195:53:AsiatechDC6"
    "185.191.79.208:53:AvaBarid2"
    "185.18.213.184:53:Sefroyek4"
    "176.65.240.82:53:IranTlcm2"
    "185.137.25.216:53:Faranet4"
    "185.53.141.35:53:MobinHost"
    "178.22.122.246:53:AsiatechDC7"
    "194.48.198.108:53:Sepehr"
    "185.161.112.34:53:ParvazSys"
    "185.129.168.241:53:Fanhab"
    "185.140.4.77:53:DourFars"
    "185.172.213.4:53:FDI"
    "194.60.210.66:53:Farzanegan"
    "5.202.100.101:53:PishToseeh"
    "185.53.141.54:53:MobinHost2"
    "185.165.119.49:53:Dadeh"
    "185.192.113.26:53:RADCOM"
    "185.82.165.10:53:ZiTEL"
)
...
benchmark_resolvers() {
    ...
    step "Benchmarking ${#IRAN_RESOLVERS[@]} resolvers (parallel)..."
    echo ""

    rm -rf "$tmpdir"
    mkdir -p "$tmpdir"

    # Launch parallel tests (5 at a time)
    local pids=() idx=0 batch_size=5
    ...
    for entry in "${IRAN_RESOLVERS[@]}"; do
        ...
        (
            ...
            # DNS query test with timing
            local start_t end_t ms=999
            start_t=$(date +%s%N 2>/dev/null || echo 0)

            local dns_ok=false
            if command -v dig &>/dev/null; then
                dig "@${ip}" -p "${port}" "${test_domain}" A +short +time=2 +tries=1 2>/dev/null | grep -qE '^[0-9]+\.[0-9]+' && dns_ok=true
            elif command -v nslookup &>/dev/null; then
                timeout "$max_wait" nslookup -port="${port}" "${test_domain}" "${ip}" 2>/dev/null | grep -qi "address" && dns_ok=true
            else
                dns_ok=true  # Can't test DNS, assume TCP was enough
            fi
    ...
    local working=${#sorted[@]}
    echo ""
    success "Found ${working} working resolvers"
    echo ""
