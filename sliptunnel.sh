...
create_server_service() {
    ...
    # ── Read the actual reset-seed VALUE (32 hex chars), not the file path ──
    ...
    if [[ -z "$reset_seed_val" ]] || [[ ${#reset_seed_val} -ne 32 ]]; then
        ...
        echo "$reset_seed_val" > "$CONFIG_DIR/reset-seed"
    fi
    substep "Reset seed: ${#reset_seed_val} hex chars ✓"

    # ── Probe binary --help to detect actual flags ──
    local help_out
    help_out=$("$server_bin" --help 2>&1 || "$server_bin" -h 2>&1 || true)

    # Show help for debugging
    substep "Binary help output:"
    echo "$help_out" | head -30 | while read -r line; do echo "    $line"; done
    echo ""

    local exec_cmd="${server_bin}"

    # DNS listen port flag
    if echo "$help_out" | grep -q -- "--listen"; then
        exec_cmd+=" --listen 0.0.0.0:${DNS_PORT}"
    elif echo "$help_out" | grep -q -- "--bind"; then
        exec_cmd+=" --bind 0.0.0.0:${DNS_PORT}"
    elif echo "$help_out" | grep -q -- "--dns-listen-port"; then
        exec_cmd+=" --dns-listen-port ${DNS_PORT}"
    elif echo "$help_out" | grep -q -- "--port"; then
        exec_cmd+=" --port ${DNS_PORT}"
    else
        # Positional: domain target [listen_addr]
        exec_cmd+=" --listen 0.0.0.0:${DNS_PORT}"
    fi
    ...
    # Target address flag
    if echo "$help_out" | grep -q -- "--target-address"; then
        exec_cmd+=" --target-address ${TARGET_ADDR}"
    elif echo "$help_out" | grep -q -- "--target"; then
        exec_cmd+=" --target ${TARGET_ADDR}"
    elif echo "$help_out" | grep -q -- "--upstream"; then
        exec_cmd+=" --upstream ${TARGET_ADDR}"
    fi

    # Cert and key
    if echo "$help_out" | grep -q -- "--cert"; then
        exec_cmd+=" --cert ${CONFIG_DIR}/cert.pem"
    fi
    if echo "$help_out" | grep -q -- "--key"; then
        exec_cmd+=" --key ${CONFIG_DIR}/key.pem"
    fi

    # Reset seed — pass INLINE VALUE, not file path
    if echo "$help_out" | grep -q -- "--reset-seed"; then
        exec_cmd+=" --reset-seed ${reset_seed_val}"
    fi
...
EOF
    ...
    if systemctl is-active --quiet slipstream-server; then
        ...
        error "Service failed to start on port ${DNS_PORT}!"
        echo ""
...
create_client_service() {
    ...
    # Local listen port flag
    if echo "$help_out" | grep -q -- "--tcp-listen-port"; then
        exec_cmd+=" --tcp-listen-port ${LOCAL_LISTEN_PORT}"
    elif echo "$help_out" | grep -q -- "--listen"; then
        exec_cmd+=" --listen 127.0.0.1:${LOCAL_LISTEN_PORT}"
    elif echo "$help_out" | grep -q -- "--bind"; then
        exec_cmd+=" --bind 127.0.0.1:${LOCAL_LISTEN_PORT}"
    elif echo "$help_out" | grep -q -- "--port"; then
        exec_cmd+=" --port ${LOCAL_LISTEN_PORT}"
    else
        exec_cmd+=" --tcp-listen-port ${LOCAL_LISTEN_PORT}"
    fi
...
KEOF
    ...
    sleep 5
    if systemctl is-active --quiet slipstream-client; then
        success "slipstream-client is RUNNING on port ${LOCAL_LISTEN_PORT}"
    else
        warn "Service may still be starting..."
        journalctl -u slipstream-client --no-pager -n 10 2>/dev/null | while read -r line; do
            echo -e "    ${DIM}${line}${RST}"
        done
...
if [[ $wd_alive -lt $MIN_ALIVE && -f "$POOL" ]]; then
    ...
    # Rebuild service ExecStart — direct flags, no --help detection
