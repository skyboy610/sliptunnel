...
auto_replenish_resolvers() {
    ...
    if [[ $alive -le 1 ]]; then
        ...
        if benchmark_resolvers 2; then
            ...
            echo "$help_out" | grep -qi "\-\-tcp-listen-port" && exec_cmd+=" --tcp-listen-port ${LOCAL_LISTEN_PORT:-5201}"
...
if [[ $wd_alive -lt $MIN_ALIVE && -f "$POOL" ]]; then
    ...
    # Rebuild service ExecStart
    wd_client="/opt/slipstream-tunnel/bin/slipstream-client"
    wd_help=$("$wd_client" --help 2>&1 || true)
    wd_exec="$wd_client"

    if echo "$wd_help" | grep -qi "\-\-tcp-listen-port"; then
        wd_exec+=" --tcp-listen-port ${LOCAL_LISTEN_PORT:-5201}"
    elif echo "$wd_help" | grep -qi "\-\-listen"; then
        wd_exec+=" --listen 127.0.0.1:${LOCAL_LISTEN_PORT:-5201}"
    fi
    echo "$wd_help" | grep -qi "\-\-domain" && wd_exec+=" --domain ${TUNNEL_DOMAIN}"
    echo "$wd_help" | grep -qi "\-\-cert" && wd_exec+=" --cert /etc/slipstream/cert.pem"
